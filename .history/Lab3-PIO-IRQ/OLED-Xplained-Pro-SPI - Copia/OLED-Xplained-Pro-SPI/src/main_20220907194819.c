...
int main(void)
{
	...
	while (1)
	{
		// using interrupts we will make two modes for the button
		// one for the first press and one for the holding press

		if(!but_flag){
			// first press
			// we will make the led blink 3 times
			pisca_led(3, 100);
			// we will print the time on the OLED
			// we will use the time.h library
			time_t t = time(NULL);
			...
			// holding press
			// we will make the led blink 10 times
			pisca_led(10, 100);
			// we will print the date on the OLED
			// we will use the time.h library
			time_t t = time(NULL);
			struct tm tm = *localtime(&t);
			sprintf(str, "%d/%d/%d", tm.tm_mday, tm.tm_mon + 1, tm.tm_year + 1900);
			gfx_mono_draw_string(str, 0, 0, &sysfont);
		}
