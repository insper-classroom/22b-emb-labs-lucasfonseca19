...
int main(void)
{
	...
	while (1)
	{
		...
		if(!but_flag){
			// we wait 1 second if it is still being pressed
			delay_ms(1000);
			// while theres no interruption we will increment delay after 1 second
			while(!but_flag){
				delay += 100;
			...
			// if it is not being pressed we will decrement delay after 1 second
			delay -= 100;
			sprintf(str, "Delay: %d", delay);
			gfx_mono_draw_string(str, 0, 0, &sysfont);
			delay_ms(1000);
