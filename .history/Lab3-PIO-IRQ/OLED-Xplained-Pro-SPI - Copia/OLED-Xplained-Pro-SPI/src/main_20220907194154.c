...
int main(void)
{
	...
	while (1)
	{
		// configure short and long press for button 1
		if (but_flag == 1)
		{
			if (previous_but_state == 1)
			{
				firstButtonPressTime = millis();
			}
			previous_but_state = 0;
			...
			if (holdingTime > 1000)
			{
				...
				gfx_mono_draw_string("Long Press", 0, 10, &sysfont);
				previousHoldingTime = holdingTime;
			}
			else
			{
				// short press
				gfx_mono_draw_string("Short Press", 0, 10, &sysfont);
			}
		}
		else
		{
			previous_but_state = 1;
		}
