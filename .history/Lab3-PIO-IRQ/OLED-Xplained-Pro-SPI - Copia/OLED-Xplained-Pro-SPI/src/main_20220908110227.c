...
int main(void)
{
	...
	{	
		...
		if (but_flag && long_press_flag == 0)
		{
			...
			delay_ms(500);
			if (but_flag) // se ainda esta pressionado
			{
				long_press_flag = 1; // ativamos a flag de clique longo
				delay += 100; // aumentamos o delay
			}
			else if (!but_flag && long_press_flag == 0)
			{
				delay -= 100;
		...
		else if (!but_flag && long_press_flag == 1)
		{
			long_press_flag = 0;
		}
