...
int main(void)
{
	...
	pio_pull_up(BUT_PIO1, BUT_IDX_MASK1, 1);
	pio_pull_up(BUT_PIO2, BUT_IDX_MASK2, 1);
	pio_pull_up(BUT_PIO3, BUT_IDX_MASK3, 1);
	// SUPER LOOP
	// aplicacoes embarcadas no devem sair do while(1).
	while(1) {
	pio_set(LED_PIO1, LED_IDX_MASK1);      // Coloca 1 no pino LED
    delay_ms(200);                        // Delay por software de 200 ms
    pio_clear(LED_PIO1, LED_IDX_MASK1);    // Coloca 0 no pino do LED
    delay_ms(200);  
	pio_set(LED_PIO2, LED_IDX_MASK2);      // Coloca 1 no pino LED
	delay_ms(200);                        // Delay por software de 200 ms
	pio_clear(LED_PIO2, LED_IDX_MASK2);    // Coloca 0 no pino do LED
	delay_ms(200);
