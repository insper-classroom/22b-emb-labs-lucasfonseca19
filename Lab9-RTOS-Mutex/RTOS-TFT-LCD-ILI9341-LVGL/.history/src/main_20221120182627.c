...
SemaphoreHandle_t xSemaphoreCLK;
/************************************************************************/
/* LCD / LVGL                                                           */
/************************************************************************/
...
void RTC_Handler(void)
{
	...
	if ((ul_status & RTC_SR_SEC) == RTC_SR_SEC)
	{
		...
		xSemaphoreGiveFromISR(xSemaphoreCLK,0);
	}
...
void lv_termostato(void)
{
	...
	lv_obj_align(btn2, LV_ALIGN_BOTTOM_LEFT, 55, 0);
	...
	lv_obj_add_event_cb(btn3, menu_handler, LV_EVENT_ALL, NULL);
	lv_obj_align(btn3, LV_ALIGN_BOTTOM_LEFT, 100, 0);
	...
	lv_obj_set_height(btn3, 45);

	/* ----------------------------------- Up ----------------------------------- */

	lv_obj_t *btn4 = lv_btn_create(lv_scr_act());
	lv_obj_add_event_cb(btn4, up_handler, LV_EVENT_ALL, NULL);
	lv_obj_align(btn4, LV_ALIGN_BOTTOM_LEFT, 210, 0);
	...
	lv_obj_set_height(btn5, 45);

	/* ---------------------------------- Floor ---------------------------------- */
	labelFloor = lv_label_create(lv_scr_act());
	lv_obj_align(labelFloor, LV_ALIGN_LEFT_MID, 35, 5);
	lv_obj_set_style_text_font(labelFloor, &dseg70, LV_STATE_DEFAULT);
	lv_obj_set_style_text_color(labelFloor, lv_color_white(), LV_STATE_DEFAULT);
	lv_label_set_text_fmt(labelFloor, "%02d", 23);
	/* ---------------------------------- Temp ---------------------------------- */
	labelSetValue = lv_label_create(lv_scr_act());
	lv_obj_align(labelSetValue, LV_ALIGN_RIGHT_MID, -35, 5);
	...
	lv_obj_align(labelClock, LV_ALIGN_RIGHT_MID, -35, -75);
	lv_obj_set_style_text_font(labelClock, &dseg20_hh_mm, LV_STATE_DEFAULT);
	lv_obj_set_style_text_color(labelClock, lv_color_white(), LV_STATE_DEFAULT);
	lv_label_set_text(labelClock, "HH:MM");
}
...
static void task_clock(void *pvParameters)
{
	...
	for (;;)
	{
		...
		if (xSemaphoreTake(xSemaphoreCLK, portMAX_DELAY) == pdTRUE)
		{
			//lv_label_set_text_fmt(labelClock, "%02d:%02d", hour, minute);
...
int main(void)
{
	...
	configure_touch();
	configure_lvgl();
	xSemaphoreCLK = xSemaphoreCreateBinary();
	...
	if (xSemaphoreCLK == NULL)
	{
		// Não foi possível criar o semáforo.
		printf("Nao foi possivel criar o semaforo.\r");
	}
