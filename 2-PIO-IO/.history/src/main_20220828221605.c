        ...
        const uint32_t ul_mask)
{
	...
	{
		ul_value = p_pio->PIO_ODSR 
	}
	else
	{
		ul_value = p_pio->PIO_PDSR
	}
	return (ul_value & ul_mask);
}
...
int main(void)
{
	...
	while (1)
	{
		if (!_pio_get(BUT_PIO1, PIO_INPUT, BUT_IDX_MASK1))
		{
			for (int i = 0; i < 10; i++)
			{
		...
		if (!_pio_get(BUT_PIO2, PIO_INPUT, BUT_IDX_MASK2))
		{
			for (int i = 0; i < 10; i++)
			{
		...
		if (!_pio_get(BUT_PIO3, PIO_INPUT, BUT_IDX_MASK3))
		{
			for (int i = 0; i < 10; i++)
			{
