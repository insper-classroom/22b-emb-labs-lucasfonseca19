...
static void clk_handler(lv_event_t * e) {
	...
	if (xSemaphoreTake(xSemaphore, 0) == pdTRUE) {
		...
		h = (char *)malloc(3);
		m = (char *)malloc(3);
		sprintf(h, "%02d", calendar.hour);
		sprintf(m, "%02d", calendar.minute);
		lv_label_set_text(labelClock, h);
		lv_label_set_text(labelClock+1, m);
		free(h);
		free(m);
	}
...
int main(void) {
	...
	/* RTC init */
	calendar t;
	// get current time and assign to t
	time_t now = time(NULL);
	t = { .year = localtime(&now)->tm_year + 1900, .month = localtime(&now)->tm_mon + 1, .day = localtime(&now)->tm_mday, .week = localtime(&now)->tm_wday, .hour = localtime(&now)->tm_hour, .minute = localtime(&now)->tm_min, .seccond = localtime(&now)->tm_sec };
	RTC_init(RTC, ID_RTC, t, RTC_IER_SECEN);
